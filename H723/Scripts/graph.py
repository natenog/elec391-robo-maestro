...
window3 = pg.GraphicsLayoutWidget(title="Velocity")
plot3 = window3.addPlot(title="Velocity")
curve3 = plot3.plot()
...
vels = deque([0]*BUF_LEN, maxlen=BUF_LEN)
...
def update():
    while ser.in_waiting > 0:
        try:
            ...
            if not line:
                break
            timestamp_str, encoderVal_str, subTarget_str, prop_str, integral_str, deriv_str, vel_str, output_str = line.split(',')
    ...
    if len(timestamps) > 1:
        ...
        print("Time:", timestamps[-1] / 1000.0, "|", "Position:", encoderVals[-1], "|", "Subtarget:", subTargets[-1], "|", "Prop:", props[-1], "|", "Int:", integrals[-1], "|", "Deriv:", derivs[-1], "|", "Vel:", vels[-1], "|", "Out:", outputs[-1])
        curve.setData(timeDelta, list(encoderVals))
        curve2.setData(timeDelta, list(subTargets))
        curve3.setData(timeDelta, list(vels))
